...
class Objective(object):
    ...
    def __call__(self, trial):
        ...
        # 中間層のユニット数の試行
        extract_num = int(trial.suggest_discrete_uniform(
            "extract_num", 30, 100, 10))
        ...
        # middle_droprate = trial.suggest_discrete_uniform(
        #     "middle_droprate", 0.0, 0.3, 0.05)

        parser = options.get_training_parser()
        args = options.parse_args_and_arch(parser, modify_parser=None)
        args.extract_num = extract_num

        valid_loss = main(args)
        return valid_loss
